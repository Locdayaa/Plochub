...
notis.new("<Color=White>PHLOC HUB MAIN<Color=/>"):Display()
notis.new("<Color=Blue>Founder: Plocdz<Color=/>"):Display() 
...
function InstantChooseGear()
    ...
    if v14 and v14.HadPoint then
        ...
        game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Phloc Hub", Text = "You Hadn't Gear", Duration = 30})
        task.wait(30)
    end
end
spawn(function()
            ...
            if TargetedPlayer or getLowestLevelPlayer() then
                ...
                repeat
                    ...
                    if TpCFrame and (TpCFrame.Position - LP.Character.HumanoidRootPart.Position).Magnitude < 300 and not PG.Main.PvpDisabled.Visible then
                        EClick()
                        chodienspamPhlocxienchetcuchungmay = true
                        AimbotPos = TpCFrame
                        SkillAim = true
                    else
                        chodienspamPhlocxienchetcuchungmay = nil
                        SkillAim = true
                        AimbotPos = nil
                    end
                ...
                chodienspamPhlocxienchetcuchungmay = false
                SkillAim = false
                AimbotPos = nil
                SeWReal = SeWFake
                TPS = false
...
spawn(function()
    while task.wait() do
        if chodienspamPhlocxienchetcuchungmay then
            EBuso()
...
function GetQuestV3()
    ...
    if v33000 == 0 then
        ...
        wait(.1)
        Notify("Phloc Hub", "Claimed Quest V3", 10)
    elseif v33000 == -1 then
        Notify("Phloc Hub", "You Not Have V2")
    end
end
...
local Window = RosieTeam:MakeWindow({Name = "Rosie Team", HidePremium = false, SaveConfig = false, ConfigFolder = "Phloc Hub Main"})
...
A:AddDropdown({Name = "Select Skill [Click Skill Enable]", Default = "", Options = skill, Callback = function(vSelectSkills)
    ...
    Notify("Phloc Hub", "Skill " .. tostring(vSelectSkills) .. ": " .. tostring(SkillSelected[vSelectSkills]) .. "")
end    
})
                ...
                if string.find(CheckR, "V1") then
                    ...
                    elseif RS.Remotes.CommF_:InvokeServer("BartiloQuestProgress", "Bartilo") == 1 then
                        ...
                        if Jeremy then
                            ...
                            Notify("Phloc Hub", "Not Found Boss, Start Hop", 10)
                            HopServer()
                        end
                        ...
                        if LP.Backpack:FindFirstChild("Flower 1") and LP.Backpack:FindFirstChild("Flower 2") and LP.Backpack:FindFirstChild("Flower 3") then
                            ...
                            if not LP.Backpack:FindFirstChild("Flower 1") and not LP.Character:FindFirstChild("Flower 1")then
                                if WS.Flower1.Transparency ~= 1 then
                                    Notify("Phloc Hub", "Collecting Flower 1", 10)
                            ...
                            elseif not LP.Backpack:FindFirstChild("Flower 2") and not LP.Character:FindFirstChild("Flower 2") then
                                if WS.Flower2.Transparency ~= 1 then
                                    Notify("Phloc Hub", "Collecting Flower 2", 10)
                                end
                            ...
                            elseif not LP.Backpack:FindFirstChild("Flower 3") and not LP.Character:FindFirstChild("Flower 3") then
                                Notify("Phloc Hub", "Collecting Flower 3", 10)
                    ...
                    if CheckR == "Human V2" then
                        ...
                        Notify("Phloc Hub", sk2, 15)
                        if #sk < 3 and CheckVerRace() == "Human V2" then
                            Notify("Phloc Hub", "Hop Server Have 3 Boss", 15)
                            task.wait(1)
                            HopServer()
                        end
                        if #sk >= 3 then
                            for aq, cg in pairs(BossQuest) do
                                ...
                                if cg and CheckVerRace() == "Human V2" then
                                    Notify("Phloc Hub", "Start Killing " .. aq .. " Boss To Up Human V3")
                    ...
                    elseif CheckR == "Skypiea V2" then
                        ...
                        if #SkypieaPlayers > 0 then
                            ...
                            Notify("Phloc Hub", "Not Found Player, Start Hop", 15)
                            HopServer()
                        end
                    ...
                    elseif CheckR == "Mink V2" then
                        ...
                        if ch and CheckR ~= "Mink V3" then
                            ...
                            Notify("Phloc Hub", "Remaning " .. 30 - CountChest .. " Chest", 7.5)
                            if CountChest >= 30 then
                                UpV3NoTween()
                                Notify("Phloc Hub", "Race Status: " .. tostring(RS.Remotes.CommF_:InvokeServer("Wenlocktoad", "info")))
                            end
                        elseif ch == nil then
                            repeat wait()
                                local ch = ChestNearF()
